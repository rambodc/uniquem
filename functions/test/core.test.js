...
test("preserves product inquiry context without undefined optional fields", () => { const result = validateInquiry({ inquiryType: "product", productName: "Elixir", name: "Product inquiry", email: "person@example.com", message: "I have a question about this product." }); assert.equal(result.inquiryType, "product"); assert.equal(result.productName, "Elixir"); assert.equal("phone" in result, false); });
